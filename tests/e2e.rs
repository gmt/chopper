...
fn prepare_reconcile_script_fixtures_in_dir(dir: &Path) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            prepare_reconcile_script_fixtures_in_dir(&path);
            continue;
        }
        if !path
            .extension()
        ...
        prepare_reconcile_script_fixture_for_alias(&path);
    }
}

fn prepare_reconcile_script_fixture_for_alias(alias_doc_path: &Path) {
    ...
    let (effective_function, script_value) = {
        ...
        // Some e2e fixtures still declare script-only reconcile wiring.
        // Auto-upgrade them so tests exercise the method-driven runtime path.
        let has_non_empty_script = reconcile
            .get("script")
            .and_then(toml::Value::as_str)
            .map(str::trim)
            .map(|value| !value.is_empty())
            .unwrap_or(false);
        if effective_function.is_none() && !had_function_key && has_non_empty_script {
            reconcile.insert(
                "function".to_string(),
    ...
    for target in targets {
        ...
        let _ = fs::copy(&script_path, target);
    }
}
...
"#,
    ...
    assert!(get_stdout.contains("\"max_use\": \"256M\""), "{get_stdout}");
    assert!(
...
args = [
  "-c",
  "printf 'EQ=<%s>\nREL=<%s>\nSHELL=<%s>\nDOLLAR=<%s>\nBRACE=<%s>\n' \"$CHOPPER_EQ\" \"$CHOPPER_REL\" \"$CHOPPER_SHELL\" \"$CHOPPER_DOLLAR\" \"$CHOPPER_BRACE\""
]
...
fn alias_list_command_reports_managed_aliases() {
    ...
    let list = run_chopper(&config_home, &cache_home, &["--list-aliases"]);
    assert!(
        list.status.success(),
        "{}",
...
#[test]
fn tui_rejects_removed_tmux_options() {
    let config_home = TempDir::new().expect("create config home");
    let cache_home = TempDir::new().expect("create cache home");

    let output = run_chopper(&config_home, &cache_home, &["--tui", "--tmux=off"]);
    assert!(!output.status.success(), "removed tmux options should fail");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("accepts no options"), "{stderr}");
}

#[test]
fn tui_rejects_unknown_tui_options() {
    let config_home = TempDir::new().expect("create config home");
    let cache_home = TempDir::new().expect("create cache home");

    let output = run_chopper(&config_home, &cache_home, &["--tui", "--tmux=maybe"]);
    assert!(!output.status.success(), "unknown tui options should fail");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("accepts no options"), "{stderr}");
}
