...
mod tests {
    ...
    #[test]
    fn store_rejects_manifest_with_invalid_exec_path_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store("invalid-store-exec", &fingerprint, &invalid_manifest)
            .expect_err("invalid exec path manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-exec");
        assert!(
            !path.exists(),
            "invalid exec path manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_duplicate_env_remove_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut invalid_manifest = Manifest::simple(PathBuf::from("echo"));
        invalid_manifest.env_remove = vec!["CACHE_DUP".to_string(), "CACHE_DUP".to_string()];

        let err = store("invalid-store-env-remove", &fingerprint, &invalid_manifest)
            .expect_err("duplicate env_remove manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-env-remove");
        assert!(
            !path.exists(),
            "duplicate env_remove manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_blank_journal_namespace_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store("invalid-store-journal", &fingerprint, &invalid_manifest)
            .expect_err("blank journal namespace manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-journal");
        assert!(
            !path.exists(),
            "blank journal namespace manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_empty_journal_namespace_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-empty-journal-namespace",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("empty journal namespace manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-empty-journal-namespace");
        assert!(
            !path.exists(),
            "empty journal namespace manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_nul_reconcile_function_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store("invalid-store-reconcile", &fingerprint, &invalid_manifest)
            .expect_err("NUL reconcile function manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-reconcile");
        assert!(
            !path.exists(),
            "NUL reconcile function manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_blank_journal_identifier_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-journal-identifier",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("blank journal identifier manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-journal-identifier");
        assert!(
            !path.exists(),
            "blank journal identifier manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_empty_journal_identifier_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-empty-journal-identifier",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("empty journal identifier manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-empty-journal-identifier");
        assert!(
            !path.exists(),
            "empty journal identifier manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_nul_env_value_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        invalid_manifest
            .env
            .insert("CACHE_ENV".to_string(), "bad\0value".to_string());

        let err = store("invalid-store-env-value", &fingerprint, &invalid_manifest)
            .expect_err("NUL env value manifest should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-env-value");
        assert!(
            !path.exists(),
            "NUL env value manifest should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_equals_env_remove_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut invalid_manifest = Manifest::simple(PathBuf::from("echo"));
        invalid_manifest.env_remove = vec!["BAD=KEY".to_string()];

        let err = store(
            "invalid-store-env-remove-equals",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("env_remove key containing '=' should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-env-remove-equals");
        assert!(
            !path.exists(),
            "env_remove key containing '=' should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_whitespace_exec_path_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-whitespace-exec",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("whitespace exec path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-whitespace-exec");
        assert!(
            !path.exists(),
            "whitespace exec path should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_whitespace_env_key_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        invalid_manifest
            .env
            .insert(" CACHE_ENV".to_string(), "value".to_string());

        let err = store(
            "invalid-store-whitespace-env-key",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("whitespace env key should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-whitespace-env-key");
        assert!(
            !path.exists(),
            "whitespace env key should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_empty_env_key_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-empty-env-key",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("empty env key should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-empty-env-key");
        assert!(
            !path.exists(),
            "empty env key should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_empty_env_remove_key_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut invalid_manifest = Manifest::simple(PathBuf::from("echo"));
        invalid_manifest.env_remove = vec![String::new()];

        let err = store(
            "invalid-store-empty-env-remove-key",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("empty env_remove key should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-empty-env-remove-key");
        assert!(
            !path.exists(),
            "empty env_remove key should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_empty_reconcile_script_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-empty-reconcile-script",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("empty reconcile script should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-empty-reconcile-script");
        assert!(
            !path.exists(),
            "empty reconcile script should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_empty_reconcile_function_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-empty-reconcile-function",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("empty reconcile function should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-empty-reconcile-function");
        assert!(
            !path.exists(),
            "empty reconcile function should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    fn store_rejects_manifest_with_whitespace_reconcile_script_and_skips_cache_write() {
        ...
        let err = store(
            "invalid-store-whitespace-reconcile-script",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("whitespace reconcile script path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-whitespace-reconcile-script");
        assert!(
            !path.exists(),
            "whitespace reconcile script path should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    #[test]
    fn store_rejects_manifest_with_nul_env_key_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store("invalid-store-nul-env-key", &fingerprint, &invalid_manifest)
            .expect_err("NUL env key should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-nul-env-key");
        assert!(!path.exists(), "NUL env key should not produce cache file");
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_nul_env_remove_key_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut invalid_manifest = Manifest::simple(PathBuf::from("echo"));
        invalid_manifest.env_remove = vec!["BAD\0REMOVE".to_string()];

        let err = store(
            "invalid-store-nul-env-remove-key",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("NUL env_remove key should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-nul-env-remove-key");
        assert!(
            !path.exists(),
            "NUL env_remove key should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_nul_journal_namespace_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-nul-journal-namespace",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("NUL journal namespace should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-nul-journal-namespace");
        assert!(
            !path.exists(),
            "NUL journal namespace should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_nul_journal_identifier_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-nul-journal-identifier",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("NUL journal identifier should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-nul-journal-identifier");
        assert!(
            !path.exists(),
            "NUL journal identifier should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    #[test]
    fn store_rejects_manifest_with_empty_exec_path_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let invalid_manifest = Manifest::simple(PathBuf::from(""));

        let err = store("invalid-store-empty-exec", &fingerprint, &invalid_manifest)
            .expect_err("empty exec path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-empty-exec");
        assert!(
            !path.exists(),
            "empty exec path should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_dot_exec_path_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store("invalid-store-dot-exec", &fingerprint, &invalid_manifest)
            .expect_err("dot exec path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-dot-exec");
        assert!(
            !path.exists(),
            "dot exec path should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_dot_component_reconcile_script_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut invalid_manifest = Manifest::simple(PathBuf::from("echo"));
        invalid_manifest.reconcile = Some(ReconcileConfig {
            script: PathBuf::from("hooks/.."),
            function: "reconcile".to_string(),
        });

        let err = store(
            "invalid-store-reconcile-script-dot-component",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("dot component reconcile script path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-reconcile-script-dot-component");
        assert!(
            !path.exists(),
            "dot component reconcile script path should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    fn store_rejects_manifest_with_dotdot_exec_path_and_skips_cache_write() {
        ...
        let err = store("invalid-store-dotdot-exec", &fingerprint, &invalid_manifest)
            .expect_err("dotdot exec path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-dotdot-exec");
        assert!(
            !path.exists(),
    ...
    #[test]
    fn store_rejects_manifest_with_dotdot_reconcile_script_path_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-dotdot-reconcile-script",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("dotdot reconcile script path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-dotdot-reconcile-script");
        assert!(
            !path.exists(),
            "dotdot reconcile script path should not produce cache file"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn store_rejects_manifest_with_trailing_slash_reconcile_script_and_skips_cache_write() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let err = store(
            "invalid-store-reconcile-script-trailing-slash",
            &fingerprint,
            &invalid_manifest,
        )
        .expect_err("trailing slash reconcile script path should be rejected on store");
        assert!(
            err.to_string()
                .contains("refusing to store invalid alias cache entry manifest"),
            "{err}"
        ...
        let path = cache_path("invalid-store-reconcile-script-trailing-slash");
        assert!(
            !path.exists(),
    ...
    #[test]
    fn load_falls_back_to_valid_legacy_when_primary_hashed_entry_is_invalid() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut invalid_primary_manifest = Manifest::simple(PathBuf::from("echo"));
        invalid_primary_manifest
            .env
            .insert("BAD=KEY".to_string(), "value".to_string());
        let invalid_primary_entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
            manifest: invalid_primary_manifest,
        };
        fs::write(
            &primary_path,
    ...
    #[test]
    fn load_falls_back_to_valid_legacy_when_primary_hashed_bytes_are_corrupted() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        fs::write(&primary_path, b"not-bincode").expect("write corrupt primary cache");
        ...
        let rebuilt_primary = fs::read(&primary_path).expect("read rebuilt hashed cache");
        assert_ne!(
            rebuilt_primary, b"not-bincode",
            "rebuilt hashed cache should not retain corrupt payload"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    #[test]
    fn stale_primary_hashed_cache_for_unsafe_alias_is_pruned_when_legacy_missing() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        store(
            alias,
            &stale_fingerprint,
            &Manifest::simple(PathBuf::from("echo")),
        )
        .expect("store stale primary cache");
        assert!(primary_path.exists(), "stale primary cache should exist");
        assert!(
            !legacy_path.exists(),
            "legacy cache should not exist before fallback check"
        );
        ...
        assert!(
            !legacy_path.exists(),
            "legacy cache should remain absent when no fallback entry exists"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    fn invalid_legacy_cache_path_for_unsafe_alias_is_pruned_without_migration() {
        ...
        assert!(
            !legacy_path.exists(),
            "invalid legacy cache file should be pruned on load"
        );
        assert!(
            !hashed_path.exists(),
            "invalid legacy cache entry should not be migrated to hashed path"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    fn load_returns_none_when_primary_and_legacy_entries_are_both_invalid() {
        ...
        let mut invalid_legacy_manifest = Manifest::simple(PathBuf::from("echo"));
        invalid_legacy_manifest.env_remove = vec!["BAD=REMOVE".to_string()];
        let invalid_legacy_entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
            manifest: invalid_legacy_manifest,
        };
        fs::write(
            &legacy_path,
        ...
        assert!(
            !primary_path.exists(),
            "invalid primary hashed cache entry should be pruned"
        );
        assert!(
            !legacy_path.exists(),
    ...
    #[test]
    fn cached_manifest_with_dotdot_token_exec_path_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let path = cache_path("unsafe-exec-dotdot-token");
        fs::create_dir_all(path.parent().expect("cache path parent")).expect("create cache dir");
        let entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
        ...
        assert!(load("unsafe-exec-dotdot-token", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached exec dotdot-token path should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    #[test]
    fn cached_manifest_with_empty_journal_identifier_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        assert!(load("unsafe-journal-empty-identifier", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached empty journal identifier should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    #[test]
    fn cached_manifest_with_empty_journal_namespace_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        assert!(load("unsafe-journal-empty-namespace", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached empty journal namespace should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    #[test]
    fn cached_manifest_with_dot_reconcile_script_token_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let path = cache_path("unsafe-reconcile-script-dot");
        fs::create_dir_all(path.parent().expect("cache path parent")).expect("create cache dir");
        let entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
        ...
        assert!(load("unsafe-reconcile-script-dot", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached reconcile script dot token should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
    ...
    #[test]
    fn cached_manifest_with_empty_env_key_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut manifest = Manifest::simple(PathBuf::from("echo"));
        manifest.env.insert(String::new(), "value".to_string());

        let path = cache_path("unsafe-empty-env-key");
        fs::create_dir_all(path.parent().expect("cache path parent")).expect("create cache dir");
        let entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
        ...
        assert!(load("unsafe-empty-env-key", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached empty env key should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn cached_manifest_with_empty_env_remove_key_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let mut manifest = Manifest::simple(PathBuf::from("echo"));
        manifest.env_remove = vec![String::new()];

        let path = cache_path("unsafe-empty-env-remove-key");
        fs::create_dir_all(path.parent().expect("cache path parent")).expect("create cache dir");
        let entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
        ...
        assert!(load("unsafe-empty-env-remove-key", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached empty env_remove key should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn cached_manifest_with_empty_reconcile_script_path_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let path = cache_path("unsafe-empty-reconcile-script-path");
        fs::create_dir_all(path.parent().expect("cache path parent")).expect("create cache dir");
        let entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
        ...
        assert!(load("unsafe-empty-reconcile-script-path", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached empty reconcile script path should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }

    #[test]
    fn cached_manifest_with_empty_reconcile_function_is_pruned() {
        let _guard = ENV_LOCK.lock().expect("lock env mutex");
        let home = TempDir::new().expect("create tempdir");
        env::set_var("XDG_CACHE_HOME", home.path());
        ...
        let path = cache_path("unsafe-empty-reconcile-function");
        fs::create_dir_all(path.parent().expect("cache path parent")).expect("create cache dir");
        let entry = CacheEntry {
            version: CACHE_ENTRY_VERSION,
            fingerprint: fingerprint.clone(),
        ...
        assert!(load("unsafe-empty-reconcile-function", &fingerprint).is_none());
        assert!(
            !path.exists(),
            "invalid cached empty reconcile function should be pruned on load"
        );
        env::remove_var("XDG_CACHE_HOME");
    }
}
