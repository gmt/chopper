...
_chopper_try_source_completion_file() {
    ...
    if [[ "$skip_chopper_generated" == "1" ]]; then
        IFS= read -r first_line < "$comp_file" || first_line=""
        if [[ "$first_line" == '# Auto-generated by chopper'* ]]; then
            return 1
        fi
    fi
...
_chopper_load_underlying_completer_from_files() {
    ...
    local xdg_dir
    local old_ifs="$IFS"
    local -a comp_dirs=("${BASH_COMPLETION_USER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/bash-completion}/completions")
